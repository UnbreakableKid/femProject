...
type User struct {
	...
	PasswordHash password  `json:"-"` //ignore this
	Bio          string    `json:"bio"`
...
func (s *PostgresUserStore) GetUserByUsername(username string) (*User, error) {
	...
	WHERE username = $1
	`
	err := s.db.QueryRow(query, username).Scan(&user.ID, &user.Username, &user.Email, &user.PasswordHash.hash, &user.Bio, &user.CreatedAt, &user.CreatedAt)
...
func (s *PostgresUserStore) GetUserToken(scope, plainTextPassword string) (*User, error) {
	...
	WHERE t.hash = $1 AND t_scope = $2 and t.expiry > $3
    `
	...
	err := s.db.QueryRow(query, tokenHash[:], scope, time.Now()).Scan(&user.ID, &user.Username, &user.PasswordHash.hash, &user.Bio, &user.CreatedAt, &user.UpdatedAt)
